    ...
    for filename in files:
        ...
        with open(filename, 'r', encoding="utf8", errors="ignore") as file:
            ...
            for i in range(len(contents)):
                ...
                if "+image" in line:
                    ...
                    if contents[i+2].rstrip() != "        var citations_url = [":
                        ...
                        print(
                            "The line after the hyphen after `prepend citations_url` must exactly be:")
                    ...
                    if contents[i+2].rstrip() != "        var citations_books = [":
                        print(filename, "failed test on line", i+3)
                        print(
                            "The line after the hyphen after `prepend citations_books` must exactly be:")
            ...
            if citations_url_running:
                print(filename, "failed test.")
                print(
                    "`prepend citations_url` was found but `Citations URL End` was not.")
                sys.exit(1)

            if citations_books_running:
                print(filename, "failed test.")
                print(
                    "`prepend citations_books` was found but `Citations Books End` was not.")
                sys.exit(1)
            ...
            for item in citations_url_dict:
                ...
                for key in optional_keys:
                    if key not in item.keys():
                        ...
                        print(
                            "You can leave the value blank but it must have all the keys.")
                        sys.exit(1)
